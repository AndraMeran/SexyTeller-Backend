...
const generateToken = (id, isRedazione, name, handle) => { //si crea un token Jwt che ogni utente riceve al momento della reg o login
...
export async function register(req, res) {
    try {
        ...
        return res.status(201).json({ //risposta al frontend con reg completata e generato token
            ...
            token: generateToken(user._id, user.isRedazione, user.name, user.handle),
...
export async function googleCallback(req, res) { //Questa funzione viene chiamata dopo che Google ha verificato l'identità dell'utente.
    try {
        ...
        const token = generateToken(user._id, user.isRedazione, user.name, user.handle)
